...
def save_result(adata, result_dir, sample_id, metrics_dict=None, cluster_key='mclust', spot_size=None, save_umap=False):
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)

    result_file = os.path.join(result_dir, f"our_{sample_id}_result.csv")
    ...
    if os.path.exists(result_file):
        ...
        columns = adata.uns.get('metrics', {}).keys()
        result_df = pd.DataFrame(index=[], columns=columns)

    if 'metrics' in adata.uns:
        for key, val in adata.uns['metrics'].items():
    ...
    if spot_size is None:
        if 'spatial' in adata.uns:
            final_spot_size = None
        else:
            final_spot_size = 100
    else:
        final_spot_size = spot_size


    plt.rcParams["figure.figsize"] = (4, 4)

    try:
        sc.pl.spatial(adata, color=cluster_key, spot_size=final_spot_size,
                      title=f"{sample_id} (ARI={adata.uns['metrics'].get('ARI', 0):.2f})",
                      show=False, frameon=False)
        plt.savefig(os.path.join(result_dir, f"our_{sample_id}_spatial.pdf"), bbox_inches='tight')
        plt.close()
    except Exception as e:
        print(f"Plotting error: {e}")

    h5ad_path = os.path.join(result_dir, f"our_{sample_id}.h5ad")
    adata.write_h5ad(h5ad_path)
    print(f"H5AD file saved to {h5ad_path}")

    if save_umap:
        try:
            sc.pl.umap(adata, color=cluster_key, title=f"{sample_id} UMAP", show=False)
            plt.savefig(os.path.join(result_dir, f"our_{sample_id}_umap.pdf"), bbox_inches='tight')
            plt.close()
        except Exception as e:
...
def cal_metric(adata, ground_truth_index):
    if ground_truth_index:
        ...
        input_feature_X = adata.uns['Spatial_Net']
        pred_labels = adata.obs['mclust']
...
def Cal_Spatial_Net(adata, rad_cutoff=None, k_cutoff=None, model='Radius', verbose=True):
    ...
    if model == 'Radius':
        ...
        for it in range(indices.shape[0]):
            KNN_list.append(pd.DataFrame(zip([it]*indices[it].shape[0], indices[it], distances[it]))) # 将每个点与其邻居的索引和距离形成一个数据框并将其添加到KNN_list中
...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='SpaFre', random_seed=2020):
